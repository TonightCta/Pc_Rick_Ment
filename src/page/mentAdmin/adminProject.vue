  ...
  <div class="query_project">
    ...
    <div class="project_con" style="position:relative;">
      ...
      <div class="project_con" v-loading="loadPro">
        <el-row class="el_con publicHover" v-for="(pro,index) in proList" :key="index">
          ...
          <el-col :span="2"><div class="projectCon">
            <i class="el-icon-user" style="color:#eb7a1d;font-size:20px;cursor:pointer;" @click="editProject(index)"></i>
            <i class="el-icon-view" style="color:#eb7a1d;font-size:20px;cursor:pointer;" @click="hasDetials(index)"></i>
            <i class="el-icon-delete" style="font-size:20px;cursor:pointer;" @click="delPro(index)"></i>
          </div></el-col>
        </el-row>
      </div>
      <!-- 蒙层 -->
...
export default {
  ...
  methods:{
    ...
    delPro(index){//删除项目
      let _vm=this;
      _vm.$confirm('此操作将删除当前项目, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let formdata=new FormData();
          formdata.append('id',_vm.proList[index].id);
          _vm.$axios.post(_vm.url+'/deleteProject',formdata).then((res)=>{
            if(res.data.code==0){
